...
import type { PagingBlockItem, PagingConfig } from './types';
...
export async function calculateSlides(
  ...
  try {
    ...
    const contentH = config.contentHeight;
    ...
    for (let sectionId = 0; sectionId < sections.length; sectionId++) {
      ...
      while (blockQueue.length > 0) {
        ...
        if (soloH <= contentH) {
          ...
          // 尝试切分到下一页
          const sliced = await sliceBlock(block, contentH, config);
          if (sliced) {
